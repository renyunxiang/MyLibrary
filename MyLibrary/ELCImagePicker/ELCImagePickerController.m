...
- (BOOL)shouldSelectAsset:(ELCAsset *)asset previousCount:(NSUInteger)previousCount
{
    ...
    if (!shouldSelect) {
        NSString *title = [NSString stringWithFormat:NSLocalizedString(@"你最多只能选择%d张照片", nil), self.maximumImagesCount];
        ...
        for(ELCAsset *elcasset in assets) {
            ...
            if(assetRep != nil) {
                if (_returnsImage) {
                    ...
                    [workingDictionary setObject:thumbImage forKey:@"UIImagePickerControllerThumbImage"];
                }
